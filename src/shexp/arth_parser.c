...
static s_arth_ast *arth_parse_band(char **start, char **end, bool *err)
{
  ...
  start[i] = NULL;
  s_arth_ast *ast = xcalloc(1, sizeof(s_arth_ast));
  *ast = ARTH_AST(ARTH_BAND, arth_parse_rec(start, start + i, err),
                  arth_parse_rec(start + i + 1, end, err));
  return ast;
}
...
static s_arth_ast *arth_parse_xor(char **start, char **end, bool *err)
{
  ...
  start[i] = NULL;
  s_arth_ast *ast = xcalloc(1, sizeof(s_arth_ast));
  *ast = ARTH_AST(ARTH_XOR, arth_parse_rec(start, start + i, err),
                  arth_parse_rec(start + i + 1, end, err));
  return ast;
}
...
static s_arth_ast *arth_parse_bor(char **start, char **end, bool *err)
{
  ...
  start[i] = NULL;
  s_arth_ast *ast = xcalloc(1, sizeof(s_arth_ast));
  *ast = ARTH_AST(ARTH_BOR, arth_parse_rec(start, start + i, err),
                  arth_parse_rec(start + i + 1, end, err));
  return ast;
}
